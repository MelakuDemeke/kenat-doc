...
export async function setUserPlan(uid, plan) {
  ...
  await (await getDb()).collection(USERS).doc(uid).update({ plan, planUpdatedAt: Date.now() });

  const snap = await (await getDb()).collection(KEYS).where("uid", "==", uid).where("revoked", "==", false).get();
  await Promise.all(
    snap.docs.map(async (doc) => {
      await doc.ref.update({ plan });
      await mirrorToRedis(doc.id, { uid, plan });
    })
  );
  return snap.size;
}
...
export async function resyncRedis() {
  ...
  await Promise.all(snap.docs.map((doc) => mirrorToRedis(doc.id, { uid: doc.data().uid, plan: doc.data().plan })));
  return snap.size;
}
