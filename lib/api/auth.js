...
async function lookupInRedis(presentedHash) {
  ...
  const { uid, plan, expiresAt } = typeof raw === "string" ? JSON.parse(raw) : raw;

  // A paid plan that has lapsed falls back to free rather than being rejected: the
  // key still works, just at free limits. Enforcing it here means expiry takes
  // effect on the next request with no scheduled job involved.
  const lapsed = expiresAt && Date.now() > expiresAt;
  return { uid, plan: lapsed ? Plans.free : (Plans[plan] ?? Plans.free), lapsed: Boolean(lapsed) };
}
