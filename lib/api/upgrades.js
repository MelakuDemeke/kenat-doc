...
import { PAID_PLANS } from "./pricing.js";
import { getEffectivePricing, PLAN_PERIOD_DAYS } from "./config.js";

const DAY_MS = 24 * 60 * 60 * 1000;

/**
 * A renewal extends from the existing expiry, not from today, so paying early does
 * not cost the customer the remaining days. A lapsed or new subscription starts now.
 */
export function nextExpiry(currentExpiry, now = Date.now()) {
  const base = currentExpiry && currentExpiry > now ? currentExpiry : now;
  return base + PLAN_PERIOD_DAYS * DAY_MS;
}
...
export async function submitUpgrade({ uid, email, name, plan, txnId, payerPhone, note }) {
  ...
  // Price is read at submission time, so a later price change never retroactively
  // alters what someone was asked to pay.
  const { plans } = await getEffectivePricing();
...
export async function reviewUpgrade({ id, status, adminEmail, adminNote }) {
  ...
  if (status === "approved") {
    ...
    const current = userDoc.data()?.planExpiresAt ?? null;
    // Only extend when renewing the same tier; switching tiers starts a fresh period.
    expiresAt = nextExpiry(userDoc.data()?.plan === request.plan ? current : null);

    await setUserPlan(request.uid, request.plan, expiresAt);
  ...
  return { ...request, id, status, expiresAt };
}
