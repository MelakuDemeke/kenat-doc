...
## Practical Usage in Kenat

The `kenat` library provides two main functions for Bahire Hasab calculations:


### Get All Bahire Hasab Values for a Year
Use `getBahireHasab(year, options)` to perform all Bahire Hasab calculations and return a comprehensive object with all results. You can pass an optional `{ lang: 'amharic' | 'english' }` for localized output. The returned object includes:

- `ameteAlem`: Years since creation (number)
- `meteneRabiet`: Quotient of ameteAlem / 4 (number)
- `evangelist`: Name of the year (string)
- `evangelistRemainder`: Remainder for evangelist cycle (number)
- `medeb`: Position in 19-year cycle (number)
- `wenber`: Adjusted medeb (number)
- `metqi`: Key lunar value (number)
- `bealeMetqiDate`: Date of proclamation (object)
- `bealeMetqiWeekday`: Weekday name (string)
- `mebajaHamer`: Sum for Nineveh calculation (number)
- `ninevehDate`: Base date for movable holidays (object)
- `newYearWeekday`: Weekday of Ethiopian New Year (string)
- `movableFeasts`: Object with all movable holidays and their calculated dates

Example options:
```js
const bahireHasab = getBahireHasab(2017, { lang: 'english' });
```

### Get a Specific Movable Holiday Date
Use `getMovableHoliday(holidayKey, year)` to directly return the Ethiopian date for any movable holiday by key. Example:

```js
import { getMovableHoliday } from 'kenat';
const easter = getMovableHoliday('TINSAYE', 2017);
console.log(easter); // { year: 2017, month: 8, day: 1 }
```

#### Supported Movable Holiday Keys

| Key              | Holiday Name         |
|------------------|---------------------|
| NINEVEH          | Fast of Nineveh     |
| ABIY_TSOME       | Great Lent          |
| DEBRE_ZEIT       | Mid-Lent            |
| HOSANNA          | Palm Sunday         |
| SIKLET           | Good Friday         |
| TINSAYE          | Easter (Fasika)     |
| RIKBE_KAHNAT     | Priests' Assembly   |
| ERGET            | Ascension           |
| PARACLETE        | Pentecost           |
| TSOME_HAWARYAT   | Apostles' Fast      |
| TSOME_DIHENET    | Dihnet Fast         |

See the Kenat source for the full list and spelling.

### Error Handling

If you pass an invalid year or holiday key, Kenat will throw an error (e.g., `UnknownHolidayError`). See the error classes section for details.
...
void main() {
  // Get the Bahire Hasab data for that year
  final bahireHasab = getBahireHasab(2017);

  print('--- Bahire Hasab for 2017 E.C. ---');
  print('Amete Alem: ${bahireHasab['ameteAlem']}'); // 7517
  print('Evangelist: ${bahireHasab['evangelist']}'); // {name: ማቴዎስ, remainder: 1}
  print('Wenber: ${bahireHasab['wenber']}'); // 11
  print('Metqi: ${bahireHasab['metqi']}'); // 29

  // The base date for all movable holidays — note the key is 'nineveh',
  // not 'ninevehDate', on the Dart side
  print('Fast of Nineveh: ${bahireHasab['nineveh']}');
  // {year: 2017, month: 6, day: 3}

  // Get the date for Easter (Fasika)
  final fasika = bahireHasab['movableFeasts']['fasika'];
  print('${fasika['name']}: ${fasika['ethiopian']}');
  // ፋሲካ: {year: 2017, month: 8, day: 12}

  // Or get just the date directly by key
  final easter = getMovableHoliday('fasika', 2017);
  print(easter); // {year: 2017, month: 8, day: 12}
}
```
...
// Get the Bahire Hasab data for that year
$bahireHasab = BahireHasab::calculate(2017);

echo "--- Bahire Hasab for 2017 E.C. ---\n";
echo "Amete Alem: {$bahireHasab['ameteAlem']}\n"; // 7517
echo "Evangelist: {$bahireHasab['evangelist']['name']}\n"; // ማቴዎስ
echo "Wenber: {$bahireHasab['wenber']}\n"; // 11
echo "Metqi: {$bahireHasab['metqi']}\n"; // 29

// The base date for all movable holidays — note the key is 'nineveh',
// not 'ninevehDate', same as the Dart port
$nineveh = $bahireHasab['nineveh'];
echo "Fast of Nineveh: {$nineveh->year}-{$nineveh->month}-{$nineveh->day}\n";
// 2017-6-3

// Get the date for Easter (Fasika)
$fasika = $bahireHasab['movableFeasts']['fasika'];
echo "{$fasika->name}: {$fasika->ethiopian->year}-{$fasika->ethiopian->month}-{$fasika->ethiopian->day}\n";
// ፋሲካ: 2017-8-12

// Or get just the date directly by key
$easter = BahireHasab::getMovableHoliday('fasika', 2017);
echo "{$easter->year}-{$easter->month}-{$easter->day}"; // 2017-8-12
```
