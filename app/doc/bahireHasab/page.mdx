...
Use `getBahireHasab(year, options)` to perform all Bahire Hasab calculations and return a comprehensive object with all results. You can pass an optional `{ lang: 'amharic' | 'english' }` for localized output.
