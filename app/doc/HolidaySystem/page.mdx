...
<Tabs
  items={[<div key="js" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><SiJavascript size={16} /> JavaScript</div>, <div key="dart" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><SiDart size={16} /> Dart</div>, <div key="py" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><SiPython size={16} /> Python</div>, <div key="php" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}><SiPhp size={16} /> PHP</div>]}
>
  <Tabs.Tab>
    ```js
  ...
  <Tabs.Tab>
    ...
    void main() {
      ...
      final muslimOrStateHolidays = getHolidaysForYear(year, {
        'filter': [HolidayTags.muslim, HolidayTags.state],
      });

      print('Found ${muslimOrStateHolidays.length} holidays that are either Muslim or State holidays.');
      // Found 9 holidays that are either Muslim or State holidays.

      // Example 3: Get holidays for a specific month
      final monthHolidays = getHolidaysInMonth(year, 1, { 'filter': HolidayTags.public });
      print(monthHolidays.length); // 3

      // Example 4: Get a single holiday by key
      final meskel = getHoliday('meskel', year);
      print(meskel?['name']); // መስቀል
    }
    ```
  </Tabs.Tab>
  <Tabs.Tab>
  ...
  <Tabs.Tab>
    ...
    // Example 2: Get holidays that are 'MUSLIM' OR 'STATE'.
    // Pass an array of tags to the filter option.
    $muslimOrStateHolidays = Holidays::getHolidaysForYear($year, [
        'filter' => [HolidayTags::MUSLIM, HolidayTags::STATE],
    ]);

    echo "Found " . count($muslimOrStateHolidays) . " holidays that are either Muslim or State holidays.";
    // Found 9 holidays that are either Muslim or State holidays.

    // Example 3: Get holidays for a specific month
    $monthHolidays = Holidays::getHolidaysInMonth($year, 1, ['filter' => HolidayTags::PUBLIC]);
    echo count($monthHolidays); // 3

    // Example 4: Get a single holiday by key
    $meskel = Holidays::getHoliday('meskel', $year);
    echo $meskel?->name; // መስቀል
    ```
  </Tabs.Tab>
</Tabs>
