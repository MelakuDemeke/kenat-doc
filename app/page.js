...
function BackgroundEffects() {
    ...
    const handleMouseMove = (e) => {
      ...
      animationFrameId = requestAnimationFrame(() => {
        ...
        if (blob1Ref.current && blob2Ref.current) {
          blob1Ref.current.style.transform = `translate(${clientX / 20}px, ${clientY / 20}px)`;
          blob2Ref.current.style.transform = `translate(${clientX / 35}px, ${clientY / 35}px)`;
        }
      });
    };
  ...
  return (
    <div className="fixed inset-0 -z-10">
      <div ref={blob1Ref} className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] bg-purple-600/20 rounded-full filter blur-3xl animate-blob transition-transform duration-700 ease-out"></div>
      <div ref={blob2Ref} className="absolute bottom-[-10%] right-[-10%] w-[500px] h-[500px] bg-sky-600/20 rounded-full filter blur-3xl animate-blob animation-delay-4000 transition-transform duration-700 ease-out"></div>
    </div>
  );
}
...
function GlassCard({ children, className = '' }) {
  return (
    <div className={`bg-clip-padding backdrop-filter backdrop-blur-md bg-zinc-800/30 border border-gray-100/20 rounded-2xl shadow-lg ${className}`}>
      {children}
    </div>
  );
}

// --- Live Amharic Date/Time Component (FIXED) ---
function LiveAmharicDateTime() {
  ...
  // On the server and initial client render, hasMounted will be false.
  if (!hasMounted) {
    // Render a placeholder to prevent layout shift.
    return <div className="mt-8 h-[28px]" />;
  }

  // This part only renders on the client after mounting.
  return (
...
// --- Feature Card uses the updated GlassCard ---
...
export default function Home() {
  ...
  return (
    <div className="bg-zinc-900 text-white selection:bg-purple-500/50 relative">
      <BackgroundEffects />
            ...
            <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-6">
              ...
              <GlassCard className="w-full sm:w-auto p-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-zinc-400">
                  <FiDownload />
                  {/* Use the hasMounted check here as well for robustness */}
                  <span className="font-bold text-white">{hasMounted ? downloads.toLocaleString() : '...'}</span>
                </div>
              </GlassCard>
            </div>
          </div>
